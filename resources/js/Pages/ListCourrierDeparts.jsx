        ...
        <table className="w-full border border-gray-200 rounded-lg overflow-hidden">
          <thead className="bg-gray-200 text-gray-700">
            <tr>
              ...
              <th className="p-3 border">Date départ</th>
              <th className="p-3 border">Destinataire</th>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((c) => (
                <tr key={c.id} className="hover:bg-gray-100">
                  <td className="border p-3 font-semibold">{c.numero}</td>

                  <td className="border p-3 text-center">{c.annee}</td>

                  <td className="border p-3">{c.objet}</td>

                  <td className="border p-3 text-center">{c.date_depart}</td>

                  <td className="border p-3">{c.destinataire_externe || "-"}</td>
                  ...
                  <td className="border p-3 text-center">
                    <button
                      onClick={() =>
                        router.visit(`/courrier-departs/create?id=${c.id}`)
                      }
